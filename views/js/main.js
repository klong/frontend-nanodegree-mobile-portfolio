...
function updatePositions() {
    ...
    for (var i = 0; i < Global_pizzaMoverItems.length; i++) {
        var phase = Math.sin(scrollOffset + Global_phases[i % 5]);
        var pos = Global_pizzaMoverItems[i].basicLeft + (100 * phase);
        Global_pizzaMoverItems[i].style.transform = "translateX( " + pos + "px)";
    }
...
document.addEventListener('DOMContentLoaded', function () {
    var cols = 8;
    var s = 256;
    // make the pizzas for animated background
    for (var i = 0; i < 25; i++) {
    ...
    // NOTE: set GLOBAL variables pizzaMoverItems, phases[]
    Global_pizzaMoverItems = document.getElementsByClassName('mover');
    // precalculate the scroll offset and five phase positions for the animation of the mover class pizzas
    Global_phases = [0, 1, 2, 3, 4]; // five phases for sin function
    Global_scrollTopOffset = document.body.scrollTop / 1250;
